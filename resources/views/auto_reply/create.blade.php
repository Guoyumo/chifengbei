...
@section('content')
    ...
    <div class="box box-success col-md-10" id="createAutoReply">
            <div class="box-header with-border">
              <h3 class="box-title">{{trans('admin.create_auto_reply')}}</h3>
            </div>
            <!-- /.box-header -->
            <!-- form start -->
            <form class="form-horizontal" action="{{url('admin/autoReplys')}}" method="post" @submit="checkForm">
            {{ csrf_field() }}
              <div class="box-body">
                <div class="form-group">
                    <label for="name" :style="{color:colorName}" class="col-sm-2 control-label">{{trans('admin.auto_reply_name')}} *</label>
                    <div class="col-sm-10">
                        <input type="text"  v-model="name" name="name" class="form-control" id="auto_reply_name" placeholder="{{trans('admin.auto_reply_name')}}">
                    </div>
                </div>
                  <div class="form-group">
                      <label for="key_words" :style="{color:colorKeywords}" class="col-sm-2 control-label">{{trans('admin.auto_reply_key_words')}} * </label>
                      <div class="col-sm-10">
                          <input type="text" v-model="key_words" name="key_words" class="form-control" id="auto_reply_key_words" placeholder="{{trans('admin.auto_reply_key_words')}}">
                      </div>
                  </div>
                <div class="form-group">
                ...
                <div class="form-group " style="max-height: 400px; overflow-y: scroll;">
                    <label for="auto_reply_content_type" :style="{color:colorContent}" class="col-sm-2 control-label">{{trans('admin.media_id')}}</label>
                    <div class="col-sm-10" v-if="showContent">
                        <textarea v-model="media_input" class="form-control" rows="3" id="reply_content" name="media_id"></textarea>
                    </div>
